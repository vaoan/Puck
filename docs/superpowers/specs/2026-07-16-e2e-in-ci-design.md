...
This design wires E2E into CI using the **Libra container model** (chosen over a
...
| 9   | `scripts/e2e.mjs` (new)                                                                 | Orchestrator adapted from Libra, simplified to one app: `loadEnv(ci)` → (docker) `supabase:docker start` → (docker) if image `puck-ci` exists `docker compose up` else `docker:build`, `waitForHttp(http://127.0.0.1:5050/, 120s)` → run `apps/web` Playwright with `TARGET_ENV`. Supports `APPS_MODE=local` (`pnpm dev`) for local runs.                                                                             | No `--app` fan-out (only web).                                                                                                                                                                                    |
| 10  | `apps/web/playwright.config.ts`                                                         | Drop `webServer: pnpm dev`; source `baseURL` from `NEXT_PUBLIC_WEB_URL ?? http://localhost:5000`. App lifecycle owned by `e2e.mjs`.                                                                                                                                                                                                                                                                                   | Matches Libra (no webServer).                                                                                                                                                                                     |
